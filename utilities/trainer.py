...
class PGTrainer(object):
    def __init__(self, args, model, env, logger):
        ...
        # --- 1. Policy Optimizer (保持不变) ---
        self.policy_optimizer = optim.RMSprop(self.behaviour_net.policy_dicts.parameters(), lr=args.policy_lrate,
                                              alpha=0.99, eps=1e-5)

        # --- 2. Value Optimizer (【关键修改】兼容新旧两种 Critic 结构) ---
        # 如果模型有 critic1_dicts (修复后的 TD3)，就把两组参数加起来一起优化
        if hasattr(self.behaviour_net, 'critic1_dicts'):
            critic_params = list(self.behaviour_net.critic1_dicts.parameters()) + \
                            list(self.behaviour_net.critic2_dicts.parameters())
            self.value_optimizer = optim.RMSprop(critic_params, lr=args.value_lrate, alpha=0.99, eps=1e-5)
        # 否则 (兼容其他算法如 DDPG)，还是用旧的 value_dicts
        else:
        ...
        # schedulers with lower-bound handling
        self.policy_scheduler = StepLR(self.policy_optimizer, step_size=self.args.lr_decay_step,
                                       gamma=self.args.lr_decay_gamma)
        self.value_scheduler = StepLR(self.value_optimizer, step_size=self.args.lr_decay_step,
                                      gamma=self.args.lr_decay_gamma)
    ...
    def lr_step(self):
        """Step LR schedulers and clamp to lr_min."""
        self.policy_scheduler.step()
        self.value_scheduler.step()

        for param_group in self.policy_optimizer.param_groups:
            if param_group['lr'] < self.args.lr_min:
                param_group['lr'] = self.args.lr_min
        for param_group in self.value_optimizer.param_groups:
            if param_group['lr'] < self.args.lr_min:
                param_group['lr'] = self.args.lr_min

        pol_lr = self.policy_optimizer.param_groups[0]['lr']
        val_lr = self.value_optimizer.param_groups[0]['lr']
        print(f"[LR] policy_lr={pol_lr:.6e}, value_lr={val_lr:.6e}")