...
from utilities.decay_noise import DecayGaussianNoise as DecayNoise
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr", type=float, default=3e-4, help="Base learning rate (if not overridden by config).")
    parser.add_argument("--lr-decay-step", type=int, default=100, help="StepLR step size in episodes.")
    parser.add_argument("--lr-decay-gamma", type=float, default=0.98, help="StepLR decay factor.")
    ...
    parser.add_argument("--ess-noise-decay", type=float, default=0.9995, help="Per-step decay factor for ESS noise.")
    ...
    # === [新增] 打印启动时间 ===
    print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Program Started")
    # ==========================
    ...
    # === [关键修复] 强制用命令行 LR 覆盖 YAML 默认值 ===
    alg_config_dict["policy_lrate"] = argv.lr
    alg_config_dict["value_lrate"] = argv.lr
    # ===============================================
    ...
    # === 【关键修复】手动注入噪声生成器 (Fix: Inject Noise Objects) ===
    train.behaviour_net.pv_noise = DecayNoise(
        args.pv_noise_start, args.pv_noise_end, args.pv_noise_decay
    )
    train.behaviour_net.ess_noise = DecayNoise(
        args.ess_noise_start, args.ess_noise_end, args.ess_noise_decay
    )
    print(f"✅ Noise Generators Attached! PV Start: {args.pv_noise_start}, ESS Start: {args.ess_noise_start}")
    ...
    # === 【优化】创建并行测试环境 (Parallel Evaluation Env) ===
    # 使用 4-8 个进程即可，避免过多占用内存，同时大幅提升测试速度
    n_test_rollout_threads = 8  
    test_env_fns = [make_env(env_config_dict, rank + 10000) for rank in range(n_test_rollout_threads)]
    test_vec_env = SubprocVecEnv(test_env_fns)
    print(f"🚀 Created {n_test_rollout_threads} parallel environments for Evaluation.")
    # ========================================================
    ...
    while train.episodes < args.train_episodes_num:
        ...
        # handle done: log mean reward and other metrics per episode per env
        for env_idx in range(args.n_rollout_threads):
            if done_batch[env_idx]:
                # 获取当前环境该回合的步数，防止除以0
                ...
                # === [关键修复] 结算并记录所有详细指标 (r_soc_limit, q_loss, etc.) ===
                for k in ep_metrics:
                ...
                # 3. 噪声衰减
                if hasattr(train.behaviour_net, "pv_noise"):
                    train.behaviour_net.pv_noise.decay_step()
                if hasattr(train.behaviour_net, "ess_noise"):
                    train.behaviour_net.ess_noise.decay_step()
                
                # 4. 重置该环境的 RNN 隐藏状态
                next_hid[env_idx] = 0.0
        ...
        # ----------------------------------------------------------------------
        # 10. 并行评估 (Parallel Evaluation) - [已修复重复触发Bug]
        # ----------------------------------------------------------------------
        # [修改] 增加判断: 且当前回合尚未进行过评估 (train.episodes != last_eval_ep)
        if train.episodes > 0 and train.episodes % args.eval_freq == 0 and train.episodes != last_eval_ep:
            last_eval_ep = train.episodes  # ⚡ [关键] 立即上锁，防止本回合后续步数重复触发
            # [旧代码] train.behaviour_net.evaluation(stat, train) -> 已废弃
            
            print(f"🔄 Starting Parallel Evaluation (Threads: {n_test_rollout_threads})...")
            eval_start_time = time.time()
            
            # 1. 重置测试环境 & 初始化统计容器
            t_obs, t_state = test_vec_env.reset()
            t_ep_rewards = np.zeros(n_test_rollout_threads)
            t_metrics = {}  # 自动收集 info 中的所有标量指标
            
            # 2. 构造 actions_avail (测试集默认全 1)
            t_actions_avail = th.tensor(env_single.get_avail_actions(), device=train.device).float()
            t_actions_avail = t_actions_avail.repeat(n_test_rollout_threads, 1, 1)
            
            # 3. 初始化 RNN 隐状态 (关键：维度必须匹配测试进程数)
            # init_hidden() 返回 (1, n_agents, hid)，需要 repeat 成 (n_test_threads, n_agents, hid)
            t_last_hid = train.behaviour_net.policy_dicts[0].init_hidden()
            t_last_hid = t_last_hid.repeat(n_test_rollout_threads, 1, 1)

            # 4. 执行一步步推演 (固定步数 args.max_steps)
            # 使用 no_grad 加速推理
            with th.no_grad():
                for t_step in range(args.max_steps):
                    t_obs_tensor = th.tensor(t_obs, device=train.device).float()
                    
                    # 获取动作：注意 status='test', exploration=False
                    t_actions, _, _, _, t_next_hid = train.behaviour_net.get_actions(
                        t_obs_tensor, status='test', exploration=False, 
                        actions_avail=t_actions_avail, target=False, last_hid=t_last_hid
                    )
                    
                    # 转换动作并执行
                    _, t_actual = translate_action(args, t_actions, env_single)
                    t_next_obs, t_next_state, t_reward, t_done, t_info = test_vec_env.step(t_actual)
                    
                    # 累加奖励
                    t_ep_rewards += t_reward
                    
                    # 更新状态
                    t_obs = t_next_obs
                    t_last_hid = t_next_hid
                    
                    # 自动累加详细指标 (Robust Metrics Collection)
                    if t_info:
                        # 初始化 key (仅第一步)
                        if not t_metrics:
                            for k, v in t_info[0].items():
                                if isinstance(v, (int, float, np.number)):
                                    t_metrics[k] = np.zeros(n_test_rollout_threads)
                        # 累加值
                        for k in t_metrics:
                            # 提取每个进程的 info[k]，如果某进程缺失则补 0
                            vals = np.array([inf.get(k, 0.0) for inf in t_info])
                            t_metrics[k] += vals

            # 5. 统计并记录到 TensorBoard
            # 计算平均每步奖励 (Mean Reward per Step)
            avg_test_reward = np.mean(t_ep_rewards) / args.max_steps
            stat['mean_test_reward'] = float(avg_test_reward)
            
            # 计算其他指标的平均值
            for k in t_metrics:
                avg_val = np.mean(t_metrics[k]) / args.max_steps
                stat['mean_test_' + k] = float(avg_val)
                
            duration = time.time() - eval_start_time
            print(f"✅ Parallel Eval Finished in {duration:.1f}s. Reward: {avg_test_reward:.4f}")
            # === [关键修复] 手动将测试数据写入 TensorBoard ===
            train.logging(stat)
            # ===============================================
        # ----------------------------------------------------------------------
        # 11. 定期保存模型 (Save Model) - [已修复重复触发Bug]
        # ----------------------------------------------------------------------
        # [修改] 增加判断: 且当前回合尚未保存过 (train.episodes != last_save_ep)
        if train.episodes > 0 and train.episodes % args.save_model_freq == 0 and train.episodes != last_save_ep:
            last_save_ep = train.episodes  # ⚡ [关键] 立即上锁
